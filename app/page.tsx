...
import { ChevronUpIcon, HomeIcon, BrainIcon, FolderIcon, AwardIcon, MailIcon, ArrowUpIcon } from "lucide-react"
    ...
    <ThemeProvider attribute="class" defaultTheme={theme} enableSystem={false}>
      ...
      <main className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-300 perspective-1000">
        ...
        <motion.div
          ...
          style={{ scaleX }}
        />
        <AnimatedThemeToggle theme={theme} onToggle={toggleTheme} /> {/* Insert the new component here */}
        ...
        <footer className="bg-gray-100 dark:bg-gray-800 py-8 border-t border-gray-200 dark:border-gray-700">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-500 text-sm mt-2">Front End Developer</p>
          </div>
        </footer>
      </main>
    </ThemeProvider>
...
function FloatingDockMenu({
  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        <IconContainer key={item.id} mouseY={mouseY} item={item} onItemClick={onItemClick} theme={theme} />
      ))}
    </motion.div>
  )
}
  ...
  return (
    <motion.div
      ...
      className={`absolute bottom-20 right-0 flex flex-col gap-2 p-4 rounded-3xl shadow-2xl border backdrop-blur-sm min-w-[220px] ${
        theme === "light" ? "bg-white/95 border-gray-200/50" : "bg-gray-900/95 border-gray-700/50"
      }`}
    >
      {items.map((item, idx) => (
        <MobileMenuItem key={item.id} item={item} index={idx} onItemClick={onItemClick} theme={theme} />
      ))}
    </motion.div>
  )
}
        ...
        <span className="text-sm font-medium">
          ...
          {isHovered && displayText.length < item.label.length && (
            <motion.span
              ...
              className={theme === "light" ? "text-emerald-500" : "text-blue-400"}
            >
              |
            </motion.span>
          )}
  ...
  return (
    <motion.button
      ...
      className={`relative flex items-center justify-center rounded-full transition-colors duration-200 ${
        theme === "light" ? "bg-gray-100 hover:bg-emerald-100" : "bg-gray-800 hover:bg-blue-800"
      }`}
      whileTap={{ scale: 0.95 }}
    >
      <AnimatePresence>
        {hovered && (
          <motion.div
            ...
            className={`absolute -top-8 left-1/2 w-fit rounded-md border px-2 py-0.5 text-xs whitespace-pre ${
              theme === "light" ? "border-gray-200 bg-white text-gray-700" : "border-gray-700 bg-gray-800 text-white"
            }`}
          >
            {item.label}
          </motion.div>
      ...
      <motion.div
        ...
        className={`flex items-center justify-center ${theme === "light" ? "text-emerald-600" : "text-blue-400"}`}
      >
